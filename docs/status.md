...
Status: in progress
Notes: Audit log helper, admin page + export, and instrumented: auction item closes, ticket/add-on/promo creation, donor CRUD, campaign CRUD, API token CRUD, webhook CRUD, sponsor CRUD, raffle CRUD, auction CRUD, voting contest CRUD, vote candidate CRUD, email template CRUD, paddle raise level CRUD.
